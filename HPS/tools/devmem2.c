...
int main(int argc, char **argv) {
    int fd; void *map_base, *virt_addr; unsigned long read_result, writeval; off_t target; int access_type = 'w';
    if(argc < 2) { fprintf(stderr, "Usage: %s { address } [ type [ data ] ]\n", argv[0]); exit(1); }
    ...
    switch(access_type) {
        ...
        default: fprintf(stderr, "Illegal data type.\n"); exit(2);
    }
    printf("Value at address 0x%lX (%p): 0x%lX\n", (unsigned long)target, virt_addr, read_result); fflush(stdout);
    if(argc > 3) {
