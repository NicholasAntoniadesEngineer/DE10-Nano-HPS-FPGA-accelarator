...
int calculator_init(void) {
    ...
    if (version == 0x0) {
        ...
        LOG_ERROR("Likely cause: DIP switch SW10 (MSEL) is misconfigured.");
        LOG_ERROR("  1. Set SW10 for HPS FPGA programming mode");
        LOG_ERROR("  2. POWER CYCLE the board (reboot is not enough — MSEL is sampled at power-on)");
        LOG_ERROR("  3. Verify: devmem2 0xff20003c w  (should return 0x00010001)");
        calculator_cleanup();
        return -1;
    }
