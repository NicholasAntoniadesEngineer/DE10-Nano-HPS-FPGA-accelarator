...
static int run_test_case(const test_case_t *test, int test_num) {
    ...
    if (ret != 0) {
        if (test->expect_error) {
            LOG_INFO("Test %d PASSED: Expected error detected correctly", test_num);
            printf("  Result:       ERROR (as expected)\n");
            printf("  %sStatus:       ✓ PASS (expected error)%s\n", COLOR_GREEN, COLOR_RESET);
    ...
    if (test->expect_error) {
        LOG_ERROR("Test %d FAILED: Expected error but operation succeeded", test_num);
        printf("  Result:       %.6f (expected error)\n", result);
        printf("  %sStatus:       ✗ FAIL (expected error, got result)%s\n", COLOR_RED, COLOR_RESET);
        return 0;  // Test failed
    }
...
int main(int argc, char *argv[]) {
    ...
    if (calculator_init() != 0) {
        ...
        printf("\nTroubleshooting:\n");
        printf("  1. DIP switch SW10 (MSEL) set for HPS FPGA programming\n");
        printf("     (power cycle required after changing — MSEL sampled at power-on)\n");
        printf("  2. FPGA bitstream loaded (boot.scr runs 'fpga load')\n");
        printf("  3. HPS-to-FPGA bridges enabled ('bridge enable' in boot.scr)\n");
        printf("  4. Running as root (needed for /dev/mem access)\n");
        printf("  Diagnostic: devmem2 0xff20003c w  (expect 0x00010001)\n");
        printf("\n");
        return 1;
    }
