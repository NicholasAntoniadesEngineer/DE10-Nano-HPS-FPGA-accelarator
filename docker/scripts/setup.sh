...
# Pull Git LFS files (kernel, bootloader binaries)
echo ""
echo -e "${BLUE}[1.8/4] Pulling Git LFS files (kernel, bootloader)...${NC}"
if command -v git-lfs &> /dev/null || git lfs version &> /dev/null 2>&1; then
    cd "$(dirname "$0")/../.." && git lfs pull && cd -
    echo -e "${GREEN}✓ Git LFS files pulled${NC}"
else
    echo -e "${YELLOW}Warning: git-lfs not installed. Install it with: brew install git-lfs${NC}"
    echo -e "${YELLOW}Without LFS the kernel and bootloader will be placeholder stubs!${NC}"
fi
